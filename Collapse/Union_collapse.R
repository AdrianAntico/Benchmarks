...
data.table::fwrite(BenchmarkResults, paste0(Path, "BenchmarkResultsCollapse_Union.csv"))
rm(BenchmarkResults)
...
funion <- function(x, y, ...) {
  ...
  if(length(ind) == 0L) return(if(is.atomic(x)) c(x, y) else rowbind(x, y))
...
BenchmarkResults <- data.table::fread(paste0(Path, "BenchmarkResultsCollapse_Union.csv"))
temp <- data[, .SD, .SDcols = c("Date","Daily Liters")]
temp1 <- copy(temp)
temp1[, `Daily Liters` := `Daily Liters` + 1]
rts <- c(rep(1.1, 10))
for(i in 1:10) {
  start <- Sys.time()
  funion(temp, temp1)
